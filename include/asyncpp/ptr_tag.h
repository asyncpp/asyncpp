...
namespace asyncpp {
	...
	std::pair<T*, size_t> ptr_untag(void* ptr) noexcept {
		...
		return {reinterpret_cast<T*>(val & ~align_mask), val & align_mask};
	}
	...
	std::pair<T*, TTag> ptr_untag(void* ptr) noexcept {
		auto [uptr, tag] = ptr_untag<T>(ptr);
		return {uptr, static_cast<TTag>(tag)};
	}
	 ...
	 */
	template<typename T>
	std::pair<const T*, size_t> ptr_untag(const void* ptr) noexcept {
		const auto align_mask = static_cast<uintptr_t>(alignof(T) - 1);
		auto val = reinterpret_cast<uintptr_t>(ptr);
		return {reinterpret_cast<const T*>(val & ~align_mask), val & align_mask};
	}
	 ...
	 */
	template<typename T, typename TTag>
	std::pair<const T*, TTag> ptr_untag(const void* ptr) noexcept {
		auto [uptr, tag] = ptr_untag<T>(ptr);
		return {uptr, static_cast<TTag>(tag)};
	}
	 ...
	 */
	template<typename... T>
	size_t ptr_get_tag(const void* ptr) noexcept {
		constexpr auto align_mask = static_cast<uintptr_t>(min_alignof<T...>() - 1);
		return reinterpret_cast<uintptr_t>(ptr) & align_mask;
	}
