	...
	public:
		...
		void push(std::function<void()> callback) override {
			...
			if (g_current_thread != nullptr) {
				...
				thread->queue.emplace(std::move(callback));
		 ...
		 */
		[[nodiscard]] size_t size() const noexcept { return m_valid_size.load(); }
		...
		struct thread_state {
			...
			std::function<void()> try_steal_task() {
				...
				for (size_t i = 0; i < pool->m_valid_size; i++) {
					...
					auto callback = std::move(thread->queue.front());
					thread->queue.pop();
					return callback;
				}
				return {};
			...
			void run() {
				...
				while (true) {
					...
					if (auto callback = try_steal_task(); callback) {
						callback();
						continue;
					}
