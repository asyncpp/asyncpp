...
namespace asyncpp {
		...
		bool await_suspend(coroutine_handle<> h) noexcept {
            ...
            while(true) {
                if(old == state_unlocked) {
                    if(mutex->m_state.compare_exchange_weak(old, state_locked_no_waiters, std::memory_order::acquire, std::memory_order::relaxed))
                        return false;
                } else {
                    next = reinterpret_cast<lock_awaiter*>(old);
                    if(mutex->m_state.compare_exchange_weak(old, reinterpret_cast<std::uintptr_t>(this), std::memory_order::release, std::memory_order::relaxed))
                        return true;
                }
            }
        }
	...
	public:
         ...
         */
		constexpr mutex_lock(class mutex& mtx, std::adopt_lock_t) noexcept : m_mtx(&mtx), m_locked{true} { assert(mtx.is_locked()); }
        /**
		...
		constexpr mutex_lock& operator=(mutex_lock&& other) noexcept {
	...
	struct [[nodiscard]] mutex::scoped_lock_awaiter {
		...
		[[nodiscard]] constexpr mutex_lock await_resume() const noexcept {
			awaiter.await_resume();
			return mutex_lock{*awaiter.mutex, std::adopt_lock};
		}
	};

	constexpr inline mutex::lock_awaiter mutex::lock() noexcept {
        return lock_awaiter{this};
    }

	constexpr inline mutex::scoped_lock_awaiter mutex::lock_scoped() noexcept {
        return scoped_lock_awaiter{this};
    }

    constexpr inline void mutex::unlock() noexcept {
        assert(m_state.load(std::memory_order::relaxed) != state_unlocked);

        auto head = m_awaiters;
        if(head == nullptr) {
            auto old = state_locked_no_waiters;
            const auto didrelease = m_state.compare_exchange_strong(old, state_unlocked, std::memory_order::release, std::memory_order::relaxed);
