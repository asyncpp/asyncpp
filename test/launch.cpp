...
TEST(ASYNCPP, AsyncLaunchScopeInvoke) {
	...
	struct destroy_tracker {
		...
		destroy_tracker(destroy_tracker&& other) noexcept : ptr(other.ptr) { other.ptr = nullptr; }
	...
	scope.invoke([t = destroy_tracker{&did_destroy}](test_dispatcher& d) -> task<void> { co_await defer{d}; }, d);
